...
public class Level {
	...
	public static boolean[][] levelTiles;
...
//					break;
			...
			for (Platform p : platforms) {
				minX = Math.min(minX, p.getTileX());
				maxX = Math.max(maxX, p.getTileX() + p.getSizeW());
				minY = Math.min(minY, p.getTileY());
				maxY = Math.max(maxY, p.getTileY() + p.getSizeH());
			}
		}
	}
